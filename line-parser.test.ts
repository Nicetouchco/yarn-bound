...
describe("line parser", () => {
  ...
  test("should handle nested markup tags", () => {
    const node = new TextResult(
      "This [someTag someProp=true][otherTag]is the[/otherTag][/someTag] end"
    );
    lineParser(node);
    expect(node.text).toBe("This is the end");
    expect(node.markup).toEqual([
  ...
  test("should handle nested markup tags", () => {
    const node = new TextResult(
      "This [someTag someProp=hello][otherTag]is[/otherTag] the[/someTag] end"
    );
    lineParser(node);
    expect(node.text).toBe("This is the end");
    expect(node.markup).toEqual([
  ...
  test("should handle overlapping markup tags", () => {
    const node = new TextResult(
      'This [someTag someProp="hello"][otherTag]is[/someTag] the[/otherTag] end'
    );
    lineParser(node);
    expect(node.text).toBe("This is the end");
    expect(node.markup).toEqual([
  ...
  test("should handle a self closing markup tag with trimwhitespace false", () => {
    const node = new TextResult(
      "This is the [someTag someProp=1 trimwhitespace=false /] end"
    );
    lineParser(node);
    expect(node.text).toBe("This is the  end");
    expect(node.markup).toEqual([
  ...
  test("should handle a close all tag", () => {
    const node = new TextResult(
      'This [someTag someProp="hello"][otherTag]is the[/] end'
    );
    lineParser(node);
    expect(node.text).toBe("This is the end");
    expect(node.markup).toEqual([
  ...
  test("should handle a nomarkup tag", () => {
    const node = new TextResult(
      "[things /] This [nomarkup]is [stuff=1] the [/stuff] end[/nomarkup]"
    );
    lineParser(node);
  ...
  test("should handle escaped brackets", () => {
    const node = new TextResult(
      "[realthings /] \\[fakethings/\\] This \\[[realstuff]is \\[fakestuff=1\\] the \\[/fakestuff\\] end[/realstuff]"
    );
    lineParser(node);
    expect(node.text).toBe(
      "[fakethings/] This [is [fakestuff=1] the [/fakestuff] end"
    );
    expect(node.markup).toEqual([
      {
        name: "realthings",
  ...
  test("should handle a select markup tag", () => {
    const node = new TextResult(
      'This is the [select value=morning morning="beginning" evening="end" /]'
    );
  ...
  test("should handle a plural markup tag", () => {
    const node = new TextResult(
      '[plural value=27 one="This is" many="These are % of"] the [plural value=27 one="end" many="ends" /]'
    );
  ...
  test("should handle an ordinal markup tag", () => {
    const node = new TextResult(
      'This is the [ordinal value=111 one="%st" two="%nd" few="%rd" other="%th" /] end'
    );
    lineParser(node);
